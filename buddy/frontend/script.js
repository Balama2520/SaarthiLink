...
class BuddyCore {
  ...
  _initEvents() {
    ...
    this.dom.modelSelect.onchange = () => {
      localStorage.setItem('buddy_prev_model', this.dom.modelSelect.value);
    };
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) this.network.abortAll();
    });
  }
  ...
  addMessage(role, text, persist = true) {
    ...
    div.innerHTML = `
      <div class="message-meta">
        <span class="meta-icon">${role === 'user' ? '👤' : '🤖'}</span>
    ...
    textEl.textContent = text; // ALWAYS use textContent to prevent XSS
  ...
  async send(text) {
    ...
    // Step 1: Neural Processing Visualization
    await this.thoughts.show();
    this.thoughts.hide();
    ...
    try {
      ...
      // Use auth helper for token-protected fetch
  ...
  cycleVoices() {
    ...
    this._updateVoiceIcon();
    this.showToast('Voice Interface', this.useVoice ? 'SPEECH UPLINK ENABLED' : 'SPEECH UPLINK DISABLED');
  }
  ...
  _initRecognition() {
    if (!('webkitSpeechRecognition' in window)) return this.dom.btnMic.classList.add('hidden');
    ...
    rec.onerror = (e) => {
      ...
      if (e.error === 'not-allowed') {
        this.showToast('Access Denied', 'Please grant microphone permissions in your browser.');
      } else if (e.error === 'no-speech') {
        this.showToast('No Speech Detected', 'Please speak louder or check your microphone.');
      } else if (e.error === 'audio-capture') {
        this.showToast('Microphone Error', 'No microphone found. Please connect a microphone.');
      } else {
        this.showToast('Mic Fault', e.error);
      }
      this.updateState('IDLE');
      this.visualizer.stop();
    };
