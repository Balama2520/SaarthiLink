...
class WorkspaceService:
    ...
    def chat_workspace(self, workspace_id: str, user_id: int, message: str, model: str):
        ...
        for r in resumes:
            if r.raw_text:
                context_parts.append(
                    f"=== GROUNDING SOURCE: Resume (Filename: {r.filename}) ===\n{r.raw_text}"
                )
        ...
        for d in docs:
            ...
            if file_text:
                context_parts.append(
                    f"=== GROUNDING SOURCE: Document (Filename: {d.filename}) ===\n{file_text}"
                )

        for n in notes:
            context_parts.append(
                f"=== GROUNDING SOURCE: AI Study Note (Title: {n.title}) ===\n{n.content}"
            )

        for j in jobs:
            context_parts.append(
                f"=== GROUNDING SOURCE: Job Application (Role: {j.job_title} at {j.company}) ===\nStatus: {j.status}\nDescription: {j.description or 'No description provided.'}"
            )
        ...
        grounding_context_text = (
            ...
            else "No files, resumes, or descriptions have been uploaded to this workspace yet."
        )
        system_prompt = PromptManager.load(
            "workspace/chat",
            workspace_name=ws.name,
            grounding_context=grounding_context_text,
        )
        ...
        async def stream_generator():
            try:
                async for chunk in AIGateway().generate_response_stream(
                    messages, model, personality="career"
                ):
