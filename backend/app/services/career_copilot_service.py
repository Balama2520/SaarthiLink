...
class CareerCopilotService:
    ...
    def compute_career_health(self, user_id: int) -> Dict[str, Any]:
        ...
        stage = (
            "Early Exploration"
            if overall < 45
            else ("Accelerating" if overall < 75 else "Job-Ready")
        )
        ...
        return {
            ...
            "recommendation": recommendations_map.get(
                biggest_gap_dim, "Continue daily mission progress."
            ),
        }
    ...
    async def analyze_skill_gap(
        self, user_id: int, target_role_override: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        matched_role_key = next(
            (k for k in ROLE_SKILL_BASELINES if k in target_role.lower()), "software"
        )
        expected_skills = ROLE_SKILL_BASELINES[matched_role_key]
