...
class RoadmapService:
    ...
    def _normalize(data: dict, target_role: str, duration_days: int) -> dict:
        ...
        for index, milestone in enumerate(milestones):
            milestone.setdefault("course_url", "https://www.coursera.org/search?query=" + target_role.replace(" ", "%20"))
            milestone.setdefault("estimated_hours", max(4, duration_days // max(1, len(milestones))))
            milestone.setdefault("completed_task_ids", [])
        return {"target_role": data.get("target_role", target_role), "duration_days": data.get("duration_days", duration_days), "milestones": milestones}
    ...
    async def generate_roadmap(self, user_id: int, target_role: str, duration_days: int) -> dict:
        ...
        try:
            ...
            if user_id != -1:
                saved = self.repo.create(LearningRoadmap(user_id=user_id, title=f"{target_role} Roadmap", target_role=target_role, duration_days=duration_days, roadmap_json=json.dumps(fallback)))
                fallback["roadmap_id"] = saved.id
            return fallback

    def update_progress(self, user_id: int, roadmap_id: str, milestone_index: int, task_index: int, completed: bool) -> dict:
        roadmap = self.repo.get_for_user(roadmap_id, user_id)
        if not roadmap:
            raise HTTPException(status_code=404, detail="Roadmap not found")
        data = json.loads(roadmap.roadmap_json or "{}")
        milestones = data.get("milestones", [])
        if milestone_index >= len(milestones) or task_index >= len(milestones[milestone_index].get("tasks", [])):
        ...
        if completed: completed_ids.add(task_index)
        else: completed_ids.discard(task_index)
