...
from app.schemas.goal import GoalCreate, GoalUpdate, GoalResponse, GoalTreeResponse, MilestoneCreate, TaskCreate
...
@router.post("/{goal_id}/milestones", response_model=GoalResponse, status_code=status.HTTP_201_CREATED)
def add_milestone(
    goal_id: str,
...
@router.post("/milestones/{milestone_id}/tasks", response_model=GoalResponse, status_code=status.HTTP_201_CREATED)
def add_task(
    milestone_id: str,
    task_in: TaskCreate,
    current_user: User = Depends(require_authenticated_user),
