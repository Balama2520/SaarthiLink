...
class FeatureRatingSubmission(BaseModel):
    feature_id: int
    rating: Union[str, int] = Field(..., description="not_useful | somewhat_useful | useful | very_useful | extremely_valuable | not_sure | 1..5")
...
def submit_feature_rating(
    ...
    if not feature_info:
        raise HTTPException(status_code=400, detail=f"Invalid feature_id {payload.feature_id}. Must be 1..34.")

    svc = DiscoveryService(db)
    profile = svc.repo.get_or_create_profile(session_id=session_id, user_id=user_id)
    svc.repo.save_feature_feedbacks(profile.id, [{
