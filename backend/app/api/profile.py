...
def _build_profile_response(profile: UserProfile, db: Session) -> dict:
    ...
    return {
        ...
        "notification_preferences": json.loads(profile.notification_preferences_json) if profile.notification_preferences_json else {},
...
def get_profile(db: Session = Depends(get_db), current_user: User = Depends(require_authenticated_user)):
...
def update_profile(updates: ProfileUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_authenticated_user)):
    profile = get_or_create_profile(current_user.id, db)
    update_data = updates.model_dump(exclude_unset=True)

    # Handle JSON fields explicitly
    json_fields = ['preferred_domains', 'preferred_industries', 'preferred_locations', 'work_preferences', 'soft_skills', 'languages', 'certifications', 'notification_preferences']
...
def replace_profile(updates: ProfileUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_authenticated_user)):
...
def get_completeness(db: Session = Depends(get_db), current_user: User = Depends(require_authenticated_user)):
    profile = get_or_create_profile(current_user.id, db)
    ...
    # Personal (3 pts)
    if profile.phone: sections["Personal"] += 33
    if profile.profile_photo_url: sections["Personal"] += 33
    if current_user.full_name: sections["Personal"] += 34
    if sections["Personal"] < 100: suggestions.append("Complete your personal details (Phone, Photo, Name).")

    # Education (5 pts)
    edu_score = 0
    if profile.degree: edu_score += 20
    if profile.university: edu_score += 20
    if profile.graduation_year: edu_score += 20
    if profile.branch: edu_score += 20
    if profile.cgpa: edu_score += 20
    sections["Education"] = edu_score
    if edu_score < 100: suggestions.append("Complete your Education section.")

    # Career (4 pts)
    car_score = 0
    if profile.target_role: car_score += 25
    if profile.career_stage: car_score += 25
    if profile.preferred_locations_json and profile.preferred_locations_json != "[]": car_score += 25
    if profile.salary_expectations: car_score += 25
    sections["Career"] = car_score
    if car_score < 100: suggestions.append("Set your Career Goals (Target Role, Locations).")
    ...
    if ts_count == 0: suggestions.append("Add Technical Skills (or let AI parse them from your resume).")
    ...
    # Portfolio
    port_score = 0
    if profile.github_url: port_score += 50
    if profile.linkedin_url: port_score += 50
    sections["Portfolio"] = port_score
    if port_score < 100: suggestions.append("Link your GitHub and LinkedIn profiles.")
...
def get_summary(db: Session = Depends(get_db), current_user: User = Depends(require_authenticated_user)):
    profile = get_or_create_profile(current_user.id, db)
    
    ts = db.query(UserSkill).filter(UserSkill.user_id == current_user.id).order_by(UserSkill.proficiency.desc()).limit(5).all()
