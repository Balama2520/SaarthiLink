...
@router.post("/register", status_code=status.HTTP_201_CREATED, summary="Register Corporate/Institutional Partner")
async def register_enterprise_org(
    ...
    """
    logger.info(f"Enterprise Registration Request: {payload.organization_name} ({payload.domain}) by User {current_user.id}")
    return {
        "status": "success",
...
async def search_enterprise_talent_pool(
    ...
    """
    logger.info(f"Enterprise Talent Search: role={role}, exp={experience} by User {current_user.id}")
    return {
        "total_matches": 1,
        "candidates": [
            {
