...
class GoalEngine:
    ...
    def expand_milestone_to_tasks(self, milestone_id: str):
        ...
        mock_tasks = [
            ...
            {"title": f"Implement core concepts of {milestone.title}", "priority": "medium"}
        ]
    ...
    def get_planning_state(self, user_id: int) -> Dict[str, Any]:
        ...
        """
        # Get active goal
        current_goal = self.db.query(Goal).filter(
            Goal.user_id == user_id, 
            Goal.type == "GOAL", 
            Goal.status == "ACTIVE"
        ).order_by(Goal.priority).first()
        
        if not current_goal:
            return {"status": "NO_ACTIVE_GOALS"}
            
        # Get active milestone
        active_milestone = self.db.query(Goal).filter(
        ...
        # Get tasks
        tasks = self.db.query(Goal).filter(
            Goal.parent_id == milestone_id,
            Goal.type == "TASK"
        ).all()
