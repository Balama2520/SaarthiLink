...
class WorkflowEngine:
    ...
    def start_workflow(self, user_id: int, workflow_type: str, initial_context: Dict[str, Any]) -> WorkflowState:
        """
        Starts a new cross-module workflow.
        """
        # Suspend any currently active workflows for this user
        active_workflows = self.db.query(WorkflowState).filter(
        ...
        self.events.publish("WorkflowStarted", {
    ...
    def advance_workflow(self, workflow_id: str, next_step: str, context_updates: Dict[str, Any]) -> WorkflowState:
        """
    ...
    def complete_workflow(self, workflow_id: str) -> WorkflowState:
        ...
        self.events.publish("WorkflowCompleted", {
            "workflow_id": workflow.id,
            "user_id": workflow.user_id,
            "type": workflow.workflow_type,
            "final_context": json.loads(workflow.context_payload) if workflow.context_payload else {}
        })
    ...
    def resume_workflow(self, workflow_id: str) -> WorkflowState:
        ...
        self.events.publish("WorkflowResumed", {
            "workflow_id": workflow.id,
            "user_id": workflow.user_id,
            "type": workflow.workflow_type
        })
