        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                async with httpx.AsyncClient(timeout=45.0) as client:
                    async with client.stream(
                        ...
                        if response.status_code != 200:
                            ...
                            raise Exception(
                                f"Gemini API error {response.status_code}: {error_str[:300]}"
                            )
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.warning(
                        "Network error calling Gemini: %s. Retrying in %.2fs...",
                        net_err,
                        backoff,
                    )
                    await asyncio.sleep(backoff)
                    ...
                    raise Exception(
                        f"Gemini network error after {MAX_RETRIES} attempts: {last_error}"
                    )
