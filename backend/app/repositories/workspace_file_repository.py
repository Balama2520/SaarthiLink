...
class WorkspaceFileRepository:
    ...
    def unlink_all_from_workspace(self, workspace_id: str) -> None:
        self.db.query(Resume).filter(Resume.workspace_id == workspace_id).update(
            {Resume.workspace_id: None}
        )
        self.db.query(Document).filter(Document.workspace_id == workspace_id).update(
    ...
    def get_document(self, item_id: str, user_id: int):
        return (
            ...
            .filter(Document.id == item_id, Document.user_id == user_id)
            .first()
        )
    ...
    def get_workspace_items(self, workspace_id: str, user_id: int):
        resumes = (
            ...
            .filter(Resume.workspace_id == workspace_id, Resume.user_id == user_id)
            .all()
        )
        docs = (
            self.db.query(Document)
            .filter(Document.workspace_id == workspace_id, Document.user_id == user_id)
            .all()
        )
        notes = (
            self.db.query(Note)
            .filter(Note.workspace_id == workspace_id, Note.user_id == user_id)
            .all()
        )
        jobs = (
            self.db.query(JobApplication)
    ...
    def get_unlinked_items(self, user_id: int):
        resumes = (
            self.db.query(Resume)
            .filter(Resume.workspace_id == None, Resume.user_id == user_id)
            .all()
        )
        docs = (
            self.db.query(Document)
