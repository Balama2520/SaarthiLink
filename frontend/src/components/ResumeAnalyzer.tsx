...
interface AnalysisResult {
  ...
  summary: string;
  word_count: number;
  analysis_source?: "ai" | "fallback" | "demo";
}
...
export default function ResumeAnalyzer() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!data?.parsed_data) {
        const message = data?.message || "AI analysis returned no parsed data. Please try again.";
        setError(message);
        toast(message, "error");
        return;
      }
      ...
      toast(message || "Resume analysis failed.", "error");
  ...
  const handleSyncProfile = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ["profileCompleteness"] });
        ...
        {result?.parsed_data && (
          <div className="space-y-6 animate-fade-in">
            <div className={`rounded-xl border px-4 py-3 text-sm ${
              result.parsed_data.analysis_source === "fallback"
                ? "border-amber-500/40 bg-amber-500/10 text-amber-200"
                : result.parsed_data.analysis_source === "demo"
                  ? "border-sky-500/40 bg-sky-500/10 text-sky-200"
                  : "border-emerald-500/40 bg-emerald-500/10 text-emerald-200"
            }`}>
              {result.parsed_data.analysis_source === "fallback"
                ? "Heuristic fallback: the AI provider did not return a valid structured response. Scores are estimated from extracted resume text."
                : result.parsed_data.analysis_source === "demo"
                  ? "Demo sample: this result is example data and is not based on your resume."
                  : "AI analysis complete: scores and recommendations were generated by the configured AI provider."}
            </div>
            {/* Score Overview */}
